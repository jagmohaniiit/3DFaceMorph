...
def normalizeImagesAndLandmarks(outSize, imIn, depthIn,pointsIn):
  h, w = outSize

  # Corners of the eye in input image
  eyecornerSrc = [pointsIn[36], pointsIn[45]]
#(np.int(pointsIn[2][0]),np.int(pointsIn[2][1])), 
#                  (np.int(pointsIn[0][0]),np.int(pointsIn[0][1]))]
  ...
  imOut = cv2.warpAffine(imIn, tform, (imIn.shape[1],imIn.shape[0]))

  depthOut = np.zeros(depthIn.shape, dtype=depthIn.dtype)
  # Apply similarity transform to input image
  depthOut = cv2.warpAffine(depthIn, tform, (w, h))
  

  return imOut, depthOut
...
def applyAffineTransform(src, srcTri, dstTri, size):
  ...
  #print("warpMat affine transform",warpMat)
